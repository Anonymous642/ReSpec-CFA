...
# windows
# arm-none-eabi-objdump -marm -dz ${projectDir}Secure/Debug/${project}_Secure.elf > Secure.asm.tmp

# linux
arm-none-eabi-objdump -dz ${projectDir}Secure/Debug/${project}_Secure.elf > ${objectsDir}Secure.asm.tmp
...
awk '{print $2}' ${objectsDir}Secure.tmp2 > ${objectsDir}Secure.mem
sed -i 's/\(.\{2\}\)/\1\n/g' ${objectsDir}Secure.mem
sed -i '/^$/d' ${objectsDir}Secure.mem

# windows
# arm-none-eabi-objdump -marm -dz ${projectDir}NonSecure/Debug/${project}_NonSecure.elf > NonSecure.asm.tmp

# linux
arm-none-eabi-objdump -dz ${projectDir}NonSecure/Debug/${project}_NonSecure.elf > ${objectsDir}NonSecure.asm.tmp
