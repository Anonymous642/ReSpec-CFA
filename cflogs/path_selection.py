...
def brute_force_subpaths(database, log, upper_bound):
    ...
    for size in range(2, stop+1):
        ...
        subdict = dict()
        for i in range(length+1-size): # check all possible subpaths of size
            subpath = log[i:i+size]
            key = " ".join(subpath)
            if key in subdict:  # If already counted, skip it
                continue
            else:
                subdict[key] = 1
                for j in range(i+1, length+1-size): # against all other possible subpaths of size
                    if subpath == log[j:size]:
                        subdict[key] += 1
                subdict[key] = ((subdict[key]*size)/length)*100
...
def detect_partial(database, path_one, path_two):
    ...
    while(index != previous):
        try:
            previous = index
            index = path_two.index(path_one[-1], index)+1
        except: 
            index = index    
         
    combined = path_one[:] 
    combined.extend(path_two[index:])  # Build larger combined path
...
def detect_overlap(database, path_one, path_two):
    ...
    if length_one <= length_two:
        if "".join(path_one) in "".join(path_two):  # Check if smaller path is contained in the larger 
            return True
        elif path_one[0] in path_two: # Check overlap at the end of larger region
            return detect_partial(database, path_two, path_one)
        elif path_one[-1] in path_two: # Check if overlap at the start of the larger region 
        ...
        if "".join(path_two) in "".join(path_one):
            return True
        elif path_two[0] in path_one:
            return detect_partial(database, path_one, path_two)
        elif path_two[-1] in path_one:
            return detect_partial(database, path_two, path_one)
        else:
            return False
...
def select_paths(database, mem_size, max_paths, partial):
    ...
    for path in paths:
        ...
        for i in range(len(results)):
            ...
            if partial and isinstance(overlap, tuple):
               sentinel = 0
               size_difference = ((2*overlap[1])+1) - ((2*results[i][1])+1) 
...
def main():
    ...
    previous, master = load_logs(args.directory)
    print("COMPLETE")
